...
export class Spectrogram {
  ...
  constructor(canvas, options) {
    let baseCanvasOptions = options.canvas || {};
  ...
  _draw(array, canvasContext) {
    ...
    let tempCanvasContext = canvasContext._tempContext;
    let tempCanvas = tempCanvasContext.canvas;
  ...
  start(offset?) {
    let source = this._sources.audioBufferStream;
    let sourceMedia = this._sources.userMediaStream;
    ...
    if (sourceMedia && sourceMedia.analyser) {
      ...
      (canvasContext as any)._tempContext = tempCanvas.getContext('2d');
